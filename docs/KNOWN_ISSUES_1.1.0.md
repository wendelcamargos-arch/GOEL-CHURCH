...
| **KI-03** | Oração/Testemunho/Servo **não postam automaticamente no grupo**. Fluxo **mantido e aceito** na 1.1.0: **mensagem pré-preenchida → WhatsApp → usuário escolhe o destino → Enviar** (`wa.me/?text=`). | **Limitação oficial do WhatsApp** (sem `text=` para grupo, sem auto-post em grupo de convite) — ver `docs/rc1/auditoria-whatsapp.md`. | Baixo — 1 toque a mais (escolher o grupo). | **V2.0** (Módulo 1 — Atendimento, via Backend + WhatsApp Business) | **Aceito** (limitação de plataforma) |
