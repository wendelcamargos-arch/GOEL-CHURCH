...
| **KI-03** | Oração/Testemunho/Servo **não postam automaticamente no grupo**. Fluxo **oficial (hotfix)** na 1.1.0: **preencher → copiar a mensagem automaticamente → abrir o grupo oficial → usuário cola e toca em Enviar**. Não há afirmação de "enviado" (o app diz "Preparamos sua mensagem"). | **Limitação oficial do WhatsApp** (sem pré-preencher/auto-post em grupo) — ver `docs/rc1/auditoria-whatsapp.md`. | Baixo — usuário cola e envia. | **V2.0** (Módulo 1 — Atendimento, via Backend + WhatsApp Business) | **Aceito** (limitação de plataforma) |
